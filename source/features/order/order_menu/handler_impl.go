...
func (h *Handler) Impl(c *gin.Context) {
	...
	if err := h.usecase.orderMenu(ctx, uint64(userID), uint64(menuID)); err != nil {
		errMSG := "Failed to order menu: " + err.Error()
		httpresputils.HttpRespBadRequest(c, &errMSG)
		return
	}

	msg := "success order menu"
	httpresputils.HttpRespAccepted(c, &msg)
}